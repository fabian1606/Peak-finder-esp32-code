...
WifiSniffer::WifiSniffer(){
    ...
    wifiSnifferPacketHandler = [this](void* buff, wifi_promiscuous_pkt_type_t type) {
    ...
    for (auto mac = macAdresses.begin(); mac != macAdresses.end();){ // go throu the list of mac adresses
        if (mac->macAdress == mac_add){ // check if the mac adress is already in the list
            ...
            this->macAdresses.erase(mac); // remove the mac adress from the list
        }
        else{
            mac++;
        }
...
void WifiSniffer::init(){
    ...
    auto cb = reinterpret_cast<wifi_promiscuous_cb_t>(wifiSnifferPacketHandler.target<void(void*, wifi_promiscuous_pkt_type_t)>()); //Convert lambda to function pointer
